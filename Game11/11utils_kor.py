'''
	...
	class InnerBoard():
		def __init__(self, width=10, height=22):
			# 폭과 길이, 단위 길이는 작은 네모난 블록의 둘레이다.
			self.width = width
			self.height = height
			self.reset()
		''' 현재 블록이 어느 위치로 이동 가능한지 판단 '''
		...
		def dropDown(self):
			...
			self.createNewTetris()
			return removed_lines
		'''블록 병합(맨 아래 모형이 더 이상 움직일 수 없는 것)'''
		...
		def removeFullLines(self):
			...
			return removed_lines
		'''새로운 블록 만들기(곧 next_tetris가 current_tetris로 변경됨)'''
		...
		def reset(self):
			...
			# 현재 블록의 회전 상태
			self.current_direction = 0
			# 현재 블록의 좌표는 단위 길이가 작은 블록의 가장자리 길이이다
			self.current_coord = [-1, -1]
			# 다음 러프 블록
			self.next_tetris = self.getNextTetris()
			# 현재 블록
