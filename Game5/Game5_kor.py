...
def show_start_interface(screen, width, height):
	...
	title = tfont.render(u'탱크 대전', True, (255, 0, 0))#탱크 전투
	content1 = cfont.render(u'1인용 게임을 원한다면 1을 누르세요', True, (0, 0, 255))#싱글플레이를 시작하려면 1을 누르십시오
	content2 = cfont.render(u'2인용 게임을 원한다면 2를 누르세요', True, (0, 0, 255))#듀오플레이를 시작하려면 2를 누르십시오
...
def show_end_interface(screen, width, height, is_win):
	...
	if is_win:
		...
		content = font.render(u'축하해！', True, (255, 0, 0))#축하해!
...
def show_switch_stage(screen, width, height, stage):
	bg_img = pygame.image.load("./images/others/background.png")
	screen.blit(bg_img, (0, 0))
	font = pygame.font.Font('./font/HMKMMAG.ttf', width//10)
	content = font.render(u'제%d단계' % stage, True, (0, 255, 0))#레벨(숫자)
	rect = content.get_rect()
	rect.midtop = (width/2, height/2)
	screen.blit(content, rect)
	pygame.display.update()
...
def main():
	...
	pygame.display.set_caption("탱크전투-공개번호:Charles의 피카츄")#탱크전투-공개번호:Charles의 피카츄
	# 이미지 로드중
	bg_img = pygame.image.load("./images/others/background.png")
	# 사운드 로드
